...
if(!empty($user_guids)){
	foreach ($user_guids as $friend_guid) {
		...
		if (!$errors) {
			// add to river
			elgg_create_river_item(array( 'view' => 'river/relationship/friend/create', 'action_type' => 'friend', 'subject_guid' => elgg_get_logged_in_user_guid(), 'object_guid' => $friend_guid,));
