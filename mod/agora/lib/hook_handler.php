...
function agora_user_settings_save() {
        include(elgg_get_plugins_path() . "agora/actions/agora/settings/usersettings/save.php");
}
...
function agora_river_menu_handler($hook, $type, $items, $params) {
	$GLOBALS['GC_THEME']->debug("agora_river_menu_handler ".var_export($params,true));
	$item = $params['item'];
		...
		if (elgg_is_admin_logged_in() || roles_has_role(elgg_get_logged_in_user_entity(),'im_admin')) {
			$post = get_entity($entity->getGuid());
				...
				$options = array(
					...
					'text' => elgg_echo('agora:remove_exec_content'),
					'title' => elgg_echo('agora:remove_exec_content'),
...
function agora_owner_block_menu_handler($hook, $type, $items, $params) {
...
function agora_profile_menu_handler($hook, $type, $items, $params) {
	$entity = $params['entity'];
...
function agora_annotation_permissions_handler($hook, $type, $result, $params) {
	$entity = $params['entity'];
	$user = $params['user'];
	$annotation_name = $params['annotation_name'];
	$GLOBALS['GC_THEME']->debug("agora_annotation_permissions_handler ".var_export($params,true));
...
function agora_container_permissions_handler($hook, $type, $result, $params) {
...
function agora_group_profile_fields($hook, $type, $fields, $params) {
	return array(
...
function gc_file_icon_url_override($hook, $type, $returnvalue, $params) {
	...
	if (elgg_instanceof($file, 'object', 'file')) {
		...
		if (preg_match('/\.docx/i',$file->originalfilename)) {
			$type='word';
			$url = "mod/agora/graphics/icons/{$type}{$ext}.gif";
...
function agora_views_add_rss_link() {
        global $autofeed;
