...
function fxuwpi_page_handler() {
        if (elgg_is_admin_logged_in()) {
        	$base_dir = elgg_get_plugins_path() . 'agora/lib';
...
function intro_upload_avatar_page_handler($page) {
		...
		echo elgg_view("agora/intro_upload_avatar");
		exit;
        }
}
function intro_join_groups_page_handler($page) {
		...
		echo elgg_view("agora/intro_join_groups");
		exit;
        }
}
function intro_add_colleagues_page_handler($page) {
		...
		echo elgg_view("agora/intro_add_colleagues");
		exit;
        }
}
function contribute_to_page_handler($page) {
		...
		echo elgg_view("agora/contribute_to");
        	// exit because this is in a modal display.
		exit;
        }
}
function notify_groups_page_handler($page) {
		...
		echo elgg_view("agora/notify_groups",array('event_guid' => $page[0]));
        	// exit because this is in a modal display.
		exit;
        }
}
function gc_event_manager_page_handler($page) {
        if (! elgg_is_logged_in()) {
        	...
        	if (isset($page[0]) && $page[0] == 'event' && isset($page[1])&& ($page[1] == 'list' || $page[1] == 'view')) {
        		$base_dir = elgg_get_plugins_path() . 'agora/pages/event';
			if (isset($page[2])) {
				if ($page[1] == 'list'){
			...
			include "$base_dir/$page[1].php";
        		...
        		$base_dir = elgg_get_plugins_path() . 'agora/pages/registrationform';
                	set_input('guid', $page[2]);
...
function gc_messages_page_handler($page) {
        if (! elgg_is_logged_in()) {
        	...
        	if (isset($page[0]) && $page[0] == 'inbox' && isset($page[1])) {
        		$base_dir = elgg_get_plugins_path() . 'agora/pages/messages';
...
function gc_site_notifications_page_handler($segments) {
        $base = elgg_get_plugins_path() . 'agora/pages/site_notifications';
...
function gc_pages_page_handler($page) {
        if (! elgg_is_logged_in()) {
			...
			elgg_load_js('jquery-treeview');
        		...
        		$base_dir = elgg_get_plugins_path() . 'agora/pages/pages';
                	set_input('guid', $page[1]);
...
function gc_notifications_page_handler($segments, $handle) {
	...
	$pages_dir = elgg_get_plugins_path() . 'agora/pages';
...
function agora_groups_page_handler($segments, $handle) {
	$pages_dir = elgg_get_plugins_path() . 'agora/pages';
	...
	switch ($segments[0]) {
		...
		default:
			...
			//global $agora_original_groups_page_handler;
			//return call_user_func($agora_original_groups_page_handler, $segments, $handle);
	}
}

function agora_profile_page_handler($page) {
	if (isset($page[0])) {
		$username = $page[0];
...
function thewire_group_page_handler($page) {
        $base_dir = elgg_get_plugins_path() . 'agora/pages/thewire';
...
function file_leftmenu_page_handler($page) {
	...
	if (! elgg_is_logged_in()) {
		forward('/dashboard');
        	...
        	$base_dir = elgg_get_plugins_path() . 'agora/pages/file';
...
function gc_members_page_handler($page) {
	if (! elgg_is_logged_in()) {
		forward('/dashboard');
        	...
        	$base = elgg_get_plugins_path() . 'agora/pages/members';
        	if (!isset($page[0])) {
                	$page[0] = 'online';
        	}
...
function gc_friends_page_handler($page_elements, $handler) {
        elgg_set_context('friends');
       	$base = elgg_get_plugins_path() . 'agora/pages/friends';
...
function all_my_groups() {
        require_once elgg_get_plugins_path() . 'agora/lib/all_my_groups.php';
        return true;
}
function event_list() {
        require_once elgg_get_plugins_path() . 'agora/lib/event_list.php';
        return true;
}
function extra_feed_comments() {
        require_once elgg_get_plugins_path() . 'agora/lib/extra_feed_comments.php';
        return true;
}

function extra_feed_replies() {
        require_once elgg_get_plugins_path() . 'agora/lib/extra_feed_replies.php';
        return true;
}
function groups_autocomplete() {
        require_once elgg_get_plugins_path() . 'agora/lib/groups_autocomplete.php';
        return true;
}
function multi_invite_autocomplete() {
        require_once elgg_get_plugins_path() . 'agora/lib/multi_invite_autocomplete.php';
        return true;
}

function agora_dashboard_handler() {
	$GLOBALS['GC_THEME']->debug("in start:agora_dahboard_handler");
        require_once elgg_get_plugins_path() . 'agora/pages/dashboard.php';
        return true;
}

function agora_activity_handler() {
        require_once elgg_get_plugins_path() . 'agora/pages/activity.php';
        return true;
}

function agora_custom_river_handler() {
	$GLOBALS['GC_THEME']->debug("in start:agora_custom_river_handler");
        require_once elgg_get_plugins_path() . 'agora/pages/custom_river.php';
        return true;
}

function agora_search_page_handler($segments, $handle) {
        $pages_dir = elgg_get_plugins_path() . 'agora/pages';
