...
if (elgg_is_admin_logged_in()) {
	...
	if (! $source_username || ! $target_username) {
		...
		$GLOBALS['GC_THEME']->FATAL('Missing source or target user names');
		forward (REFERER);
	} else {
		echo "<br>".elgg_echo("source_username=$source_username target_username=$target_username");
		$GLOBALS['GC_THEME']->FATAL("source_username=$source_username target_username=$target_username");
		...
		if (!  $source_user_guid) {
			...
			$GLOBALS['GC_THEME']->FATAL("Missing source user GUID for $source_username");
			forward (REFERER);
		} elseif (!  $target_user_guid) {
			echo "<br>".elgg_echo("Missing target user GUID for $target_username");
			$GLOBALS['GC_THEME']->FATAL("Missing target user GUID for $target_username");
			forward (REFERER);
		} else {
			echo "<br>".elgg_echo("source_user_guid=$source_user_guid target_user_guid=$target_user_guid");
			$GLOBALS['GC_THEME']->FATAL("source_user_guid=$source_user_guid target_user_guid=$target_user_guid");
			echo "<br>".elgg_echo("Processing entities for $source_username");
			$GLOBALS['GC_THEME']->FATAL("Processing entities for $source_username");
			...
			foreach ($entities as $entity) {
				...
				if ($entity->save()) {
					...
					$GLOBALS['GC_THEME']->FATAL("Transferred entity ownership to $target_username for entity $entity->guid $entity->type $entity->subtype");
				} else {
					...
					$GLOBALS['GC_THEME']->FATAL("Could not transfer entity ownership to $target_username for entity $entity->guid $entity->type $entity->subtype");
				}
			}
			echo "<br>".elgg_echo("Processing metadata for $source_username");
			$GLOBALS['GC_THEME']->FATAL("Processing metadata for $source_username");
			...
			foreach ($metadatas as $metadata) {
				...
				if ($metadata->save()) {
					...
					$GLOBALS['GC_THEME']->FATAL("Transferred metadata ownership to $target_username for metadata $metadata->id $metadata->entity_guid");
				} else {
					echo "<br>".elgg_echo("Could not transfer metadata ownership to $target_username for metadata $metadata->id $metadata->entity_guid");
					$GLOBALS['GC_THEME']->FATAL("Could not transfer metadata ownership to $target_username for metadata $metadata->id $metadata->entity_guid");
				}
			}
			echo "<br>".elgg_echo("Processing annotations for $source_username");
			$GLOBALS['GC_THEME']->FATAL("Processing annotations for $source_username");
			...
			foreach ($annotations as $annotation) {
				...
				if ($annotation->save()) {
					...
					$GLOBALS['GC_THEME']->FATAL("Transferred annotation ownership to $target_username for entity $annotation->name $annotation->value $annotation->value_type");
				} else {
					...
					$GLOBALS['GC_THEME']->FATAL("Could not transfer annotation ownership to $target_username for entity $annotation->name $annotation->value $annotation->value_type");
				}
			}
			echo "<br>".elgg_echo("Processing river items for $source_username");
			$GLOBALS['GC_THEME']->FATAL("Processing river items for $source_username");
			...
			foreach ($river_items as $river_item) {
				...
				if (update_data($update)) {
					...
					$GLOBALS['GC_THEME']->FATAL("Transferred river item ownership to $target_username for river item $river_item->id $river_item->type $river_item->subtype $river_item->action_type");
				} else {
					...
					$GLOBALS['GC_THEME']->FATAL("Could not transfer river item ownership to $target_username for river item $river_item->id $river_item->type $river_item->subtype $river_item->action_type");
				}
			}
			echo "<br>".elgg_echo("Processing source relationships for $source_username");
			$GLOBALS['GC_THEME']->FATAL("Processing source relationships for $source_username");
			...
			foreach ($source_relationships as $source_relationship) {
				if(! check_entity_relationship($target_user_guid,$source_relationship->relationship,$source_relationship->guid_two)) {
					...
					if (update_data($update)) {
						...
						$GLOBALS['GC_THEME']->FATAL("Transferred relationship to $target_username for relationship $source_relationship->id $source_relationship->guid_one $source_relationship->relationship $source_relationship->guid_two");
					} else {
						...
						$GLOBALS['GC_THEME']->FATAL("Could not transfer relationship to $target_username for relationship $source_relationship->id $source_relationship->guid_one $source_relationship->relationship $source_relationship->guid_two");
					}
				}
			}
			echo "<br>".elgg_echo("Processing target relationships for $source_username");
			$GLOBALS['GC_THEME']->FATAL("Processing target relationships for $source_username");
			...
			foreach ($target_relationships as $target_relationship) {
				if(! check_entity_relationship($target_relationship->guid_one,$target_relationship->relationship,$target_user_guid)) {
					...
					if (update_data($update)) {
						...
						$GLOBALS['GC_THEME']->FATAL("Transferred relationship to $target_username for relationship $target_relationship->id $target_relationship->guid_one $target_relationship->relationship $target_relationship->guid_two");
					} else {
						...
						$GLOBALS['GC_THEME']->FATAL("Could not transfer relationship to $target_username for relationship $target_relationship->id $target_relationship->guid_one $target_relationship->relationship $target_relationship->guid_two");
					}
				}
			}
			echo "<br>".elgg_echo("Processing collections for $source_username");
			$GLOBALS['GC_THEME']->FATAL("Processing collections for $source_username");
			...
			foreach ($collection_objects as $collection_object) {
				...
				if (update_data($update)) {
					...
					$GLOBALS['GC_THEME']->FATAL("Transferred access collection ownership to $target_username for collection $collection->id $collection->name");
				} else {
					...
					$GLOBALS['GC_THEME']->FATAL("Could not transfer access collection ownership to $target_username for collection $collection->id $collection->name");
				}
			}
			echo "<br>".elgg_echo("Processing collection memberships for $source_username");
			$GLOBALS['GC_THEME']->FATAL("Processing collection memberships for $source_username");
			...
			foreach ($collection_ids as $collection_id) {
				if (update_access_collection($collection_id,$target_user_guid)) {
					...
					$GLOBALS['GC_THEME']->FATAL("Updated access collection for collection $collection_id");
				} else {
					echo "<br>".elgg_echo("Could not update access collection for collection $collection_id");
					$GLOBALS['GC_THEME']->FATAL("Could not update access collection for collection $collection_id");
				}
			}
		}
	}
