...
function agora_init() {
	...
	//elgg_register_js('jquery.scrollabletab', '/mod/agora//vendors/jquery.scrollabletab.js','head');
	elgg_register_js('jquery.scrollto', '/mod/agora/vendors/jquery.scrollTo-1.4.3.1-min.js','head');
	elgg_register_js('jquery.scrollabletab', '/mod/agora/vendors/simplescrolltab.js','head');
	//elgg_register_js('jquery.jeditable', '/mod/agora/vendors/jquery.jeditable.mini.js','head');
	elgg_register_js('jquery.multidatespicker', '/mod/agora/vendors/jquery-ui.multidatespicker/jquery-ui.multidatespicker.js','head');
	elgg_register_css('multidatespicker', '/mod/agora/vendors/jquery-ui.multidatespicker/css/mdp.css','head');
        elgg_load_css('multidatespicker');
	//elgg_register_js('jquery.tinyscrollbar', '/mod/agora/vendors/jquery.tinyscrollbar.js','head');
	//elgg_register_js('jquery.scrollabletab', '/mod/agora/vendors/jquery.scrollabletab.js','head');
	...
	// Add wire
	add_group_tool_option('thewire',elgg_echo('agora:enable_group_wire'),TRUE);
	...
	elgg_register_plugin_hook_handler('output:before', 'layout', 'agora_views_add_rss_link');
	 ...
	 * Customize pages
	 */
	elgg_register_plugin_hook_handler('index', 'system', 'agora_index_handler');
	...
	elgg_register_page_handler('profile', 'agora_profile_page_handler');
	elgg_register_page_handler('dashboard', 'agora_dashboard_handler');
	elgg_unregister_page_handler('search', 'search_page_handler');
	elgg_register_page_handler('search', 'agora_search_page_handler');
	...
	//global $CONFIG, $agora_original_groups_page_handler;
	//$agora_original_groups_page_handler = $CONFIG->pagehandler['groups'];
	elgg_register_page_handler('groups', 'agora_groups_page_handler');
	//elgg_register_ajax_view('blog/composer');
	elgg_register_ajax_view('agora/ajax/add_gc_comment');
	elgg_register_ajax_view('agora/ajax/view/comment');
	elgg_register_ajax_view('agora/ajax/edit_gc_wire');
	elgg_register_ajax_view('agora/ajax/edit_gc_gft');
	elgg_register_ajax_view('agora/ajax/add_discussion_reply');
	elgg_register_ajax_view('agora/ajax/view/discussion_reply');
	...
	elgg_register_ajax_view('agora/ajax/friendsof');
	elgg_register_ajax_view('agora/ajax/file_friends');
	elgg_register_ajax_view('agora/ajax/file_owner');
	elgg_register_ajax_view('agora/ajax/file_world');
	elgg_register_ajax_view('agora/ajax/group_members');
	elgg_register_ajax_view('agora/ajax/group_wall');
	...
	elgg_register_ajax_view('agora/ajax/pages_world');
	elgg_register_ajax_view('agora/ajax/polls');
	elgg_register_ajax_view('agora/ajax/search');
	elgg_register_ajax_view('agora/ajax/thewire');
	elgg_register_ajax_view('agora/ajax/user_activity');
	elgg_register_ajax_view('agora/ajax/user_colleagues');
	elgg_register_ajax_view('agora/ajax/user_groups');
	...
	elgg_register_js('elgg.all_my_groups', '/mod/agora/js/lib/all_my_groups.js');
	elgg_register_js('elgg.extra_feed_comments', '/mod/agora/js/lib/extra_feed_comments.js');
	elgg_register_js('elgg.extra_feed_discussion_replies', '/mod/agora/js/lib/extra_feed_discussion_replies.js');
	elgg_register_js('elgg.activity_stream', '/mod/agora/js/lib/ui.activity_stream.js');
	elgg_register_js('elgg.new_feeds', '/mod/agora/js/lib/new_feeds.js');
	elgg_register_js('elgg.new_messages', '/mod/agora/js/lib/new_messages.js');
	elgg_register_js('elgg.gcblog', '/mod/agora/views/default/js/gcblog.js');
	elgg_register_js('elgg.user_handle', '/mod/agora/js/lib/user_handle.js');
	elgg_register_js('elgg.contributed_by', '/mod/agora/js/lib/contributed_by.js');
	elgg_register_js('elgg.toggle_cla', '/mod/agora/js/lib/toggle_cla.js');
	//elgg_register_js('elgg.scroll', '/mod/agora/js/lib/scroll.js');
	...
	elgg_unextend_view('core/settings/statistics', 'profile_manager/account/login_history');
        ...
        elgg_register_plugin_hook_handler('usersettings:save', 'user', 'agora_user_settings_save');


	$action_path = $plugins_path . 'agora/actions';
	...
	elgg_register_action("agora/notify_groups", "$action_path/agora/notify_groups.php");
	...
	elgg_register_action('agora/settings/save',$plugins_path . 'agora/actions/plugins/settings/save.php');
	//elgg_register_action('',$CONFIG->pluginspath . 'agora/settings/usersettings/save.php');
	elgg_register_action('agora/settings/save',$plugins_path . 'agora/actions/plugins/settings/save.php');
	...
	elgg_register_action("agora/pns", $action_path."/agora/pns.php");
	...
	$action_path = $plugins_path . "agora/actions/reportedcontent";
	...
	$action_path = $plugins_path . "agora/actions/groups/membership";
	...
	elgg_register_plugin_hook_handler('register', 'menu:river', 'agora_river_menu_handler');
	...
	elgg_register_plugin_hook_handler('register', 'menu:owner_block', 'agora_owner_block_menu_handler', 600);
	elgg_register_plugin_hook_handler('register', 'menu:profile', 'agora_profile_menu_handler');
        elgg_register_plugin_hook_handler('register', 'menu:entity', 'event_entity_menu_setup');

	
	elgg_register_event_handler('pagesetup', 'system', 'agora_pagesetup_handler', 1000);
	
	/**
	 * Customize permissions
	 */
	elgg_register_plugin_hook_handler('permissions_check:annotate', 'all', 'agora_annotation_permissions_handler');
	//elgg_register_plugin_hook_handler('container_permissions_check', 'all', 'agora_container_permissions_handler');
	
	/**
	 * Miscellaneous customizations
	 */
	//Small "correction" to groups profile -- brief description makes more sense to come first!
	elgg_register_plugin_hook_handler('profile:fields', 'group', 'agora_group_profile_fields', 1);
	// menu for pages
	...
	//elgg_extend_view('js/elgg', '/mod/agora/js/lib/extra_feed_comments.js');
	//elgg_extend_view('js/elgg', '/mod/agora/js/lib/extra_feed_replies.js');
