...
if ($guid) {
	...
	$file_selected = elgg_echo('agora:no_file_selected');
	$add_to_river = elgg_view('input/checkbox',array('name'=>'add_to_river','value'=>'true'));
	$add_to_river = '<div class="gc-addtoriver-row"><div class="gc-input-file-2em"><label>'.elgg_echo('agora:show_in_feed').'</label></div><div class="gc-addtoriver-cb">'.$add_to_river.'</div><span class="elgg-icon elgg-icon-info elgg-icon-info-top" title="'.elgg_echo('agora:show_in_feed:help').'"></span></div>';
}
?>
...
function check_file(){
	...
	if (<?php echo $embed; ?>){
        ...
        if(ext && $.inArray(ext, allowed_extensions) == -1) {
                alert(elgg.echo('agora:file:not_allowed'));
                $('.elgg-input-file').val('');
        } else {
...
if(file_tools_use_folder_structure()){
	...
	?>
		<div id='agora-parent-folder-label' class="gc-input-file-1em"><label><?php echo elgg_echo("file_tools:forms:edit:parent"); ?></label></div>
