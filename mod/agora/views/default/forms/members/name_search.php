...
$(document).ready(function() {
		...
		if ($.trim($("#elgg-input-searchgroup").val())) {
                        $('#member-search-submit .button-text').css('background','url('+elgg.get_site_url()+'mod/agora/views/default/images/icon-search-active.png) no-repeat 0 0');
                        $('#member-search-submit').css('pointer-events','all');
                        $('#member-search-submit').removeAttr('disabled','disabled');
                } else {
                        $('#member-search-submit .button-text').css('background','url('+elgg.get_site_url()+'mod/agora/views/default/images/icon-search-inactive.png) no-repeat 0 0');
                        $('#member-search-submit').css('pointer-events','none');
                        $('#member-search-submit').attr('disabled','disabled');
                };
        });
