...
$content = "<div class='agora-intro'>";
$content .= elgg_echo('agora:intro');
$content .= elgg_echo('agora:intro:add_colleagues');
$content .= elgg_view('output/url', array(
			'text' => elgg_echo('agora:intro:add_colleagues:title'),
			'href' => $site_url.'intro_add_colleagues',
			'class' => 'elgg-lightbox elgg-button elgg-button-submit elgg-button-fancybox',
			'id' => 'agora-intro-addcolleagues',
			));
$content .= elgg_echo('agora:intro:add_colleagues2');
//Groups
$content .= elgg_echo('agora:intro:join_groups');
...
if ( $tell_us_group ||  $voice_up_group) {
	if (! $tell_us_group->isMember($user) || ! $voice_up_group->isMember($user)) {
		$content .= elgg_echo('agora:intro:join_groups2');
		$content .= '<ul>';
		if ( $tell_us_group) {
			if (! $tell_us_group->isMember($user)) {
				...
				$content .= "<li id='intro-tell-us-join'>".elgg_view('output/url',array('text' => elgg_echo('agora:intro:tell_us'),'href'=>$tell_us_group->getURL(),'is_trusted' => true,)).' '.$join_button.'</li>';
			}
		}
		if ( $voice_up_group) {
			if (! $voice_up_group->isMember($user)) {
				...
				$content .= '<li>'.elgg_view('output/url',array('text' => elgg_echo('agora:intro:voice_up'),'href'=>$voice_up_group->getURL(),'is_trusted' => true,)).' '.$join_button.'</li>';
			}
		}
		$content .= '</ul>';
	}
}
$content .= elgg_echo('agora:intro:join_groups3');
$content .= '<p>'.elgg_view('output/url', array(
			'text' => elgg_echo('agora:intro:join_group:title'),
			'href' => $site_url.'intro_join_groups',
			'class' => 'elgg-lightbox elgg-button elgg-button-submit elgg-button-fancybox',
			'id' => 'agora-intro-joingroup',
			)).'</p>';
//Profile
$content .= elgg_echo('agora:intro:profile_setup');
$content .= '<p>'.elgg_view('output/url', array(
			'text' => elgg_echo('agora:intro:upload_avatar:title'),
			'href' => $site_url.'intro_upload_avatar',
			'class' => 'elgg-lightbox elgg-button elgg-button-submit elgg-button-fancybox',
			'id' => 'agora-intro-uploadavatar-button',
			)).'</p>';
$profile_url = "<a href=\"${site_url}profile/$user->username\">".elgg_echo('profile')."</a>";
$content .= elgg_echo('agora:intro:profile_setup2',array($profile_url));
//Videos
$content .= elgg_echo('agora:intro:training_videos');
$content .= elgg_echo('agora:intro:training_videos2');
$content .= "</div>";
echo $content;
