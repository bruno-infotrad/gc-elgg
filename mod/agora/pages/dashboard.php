...
switch ($page_type) {
		...
		if (!$stream) {
			$stream = '<h3>' . elgg_echo('agora:exec_content:none') . '</h3>';
		}
		break;
        case 'my_groups':
		...
		if (count($group_list)) {
			...
			$stream = '<h3>' . elgg_echo('agora:mygroups:none') . '</h3>';
		}
                break;
        case 'groups':
...
*/
	...
	case 'intro':
		...
		if ($count == 0) {
			$stream = '<h3>' . elgg_echo('agora:colleagues:none') . '</h3>';
		} else {
			$options['count']=FALSE;
			$stream = elgg_list_river($options);
		}
...
function get_exec_content() {
	...
	if ($exec_posts) {
		...
		$exec_posts_html .= elgg_echo('agora:exec_content_title');
		$exec_posts_html .= '</div>';
		$exec_posts_html .= '<div class="bandeau-exec-content">';
		foreach ($exec_posts as $exec_post) {
			$exec_posts_html .= elgg_view('agora/exec_content',array('entity' => $exec_post));
		}
		$exec_posts_html .= '</div>';
	}
