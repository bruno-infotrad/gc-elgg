...
//Advanced params
$advanced_type = get_input('advanced_type');
$contributed_by = get_input('contributed_by');
$approximate_date = get_input('approximate_date');
list($year,$month,$day) = split("-",$approximate_date);
$unix_timestamp = '';
if ($year && $month && $day) {
	//$unix_time_stamp = date("Y-m-d", strtotime("$approximate_date"));
	$unix_timestamp = strtotime("$approximate_date");
}
...
// Have to do this to prevent retutning tags etc
$entity_subtype = get_input('entity_subtype', ELGG_ENTITIES_ANY_VALUE);
if ($entity_subtype) {
	$entity_type = 'object';
	$search_type = 'entities';
} elseif ($advanced_type == 'user') {
// To prevent search from returning object posted by user)
	$entity_type = 'user';
	$search_type = 'entities';
} 
...
// Have to do this after params initialization
$params['advanced_type'] = $advanced_type;
$params['contributed_by'] = $contributed_by;
$params['approximate_date'] = $unix_timestamp;
$params['additional_keywords'] = $additional_keywords;
if ($advanced_type == 'groups_content') {
	$params['gc_group_content_joins'] = "JOIN " . $db_prefix . "groups_entity ge ON e.container_guid=ge.guid";
}
...
if ($search_type == 'all' || $search_type == 'entities') {
	...
	if ($search_type == 'all' || ($search_type == 'entities' && $entity_type == 'group')) {
		$type = 'group';
