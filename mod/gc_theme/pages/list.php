	...
	if(!empty($page_owner) && (elgg_instanceof($page_owner, "user") || elgg_instanceof($page_owner, "group"))) {
		...
		if(!$draw_page) {
			...
			if(elgg_instanceof($page_owner, "user")){
				...
				//echo elgg_view_page($title_text, elgg_view_layout("two_sidebar_gc", $params));
			} else {
				$params["filter"] = false;
				echo elgg_view_page($title_text, elgg_view_layout("two_sidebar_gcgroup", $params));
			}
			
			// Move into if statement because two_sidebar_gc looks for group profile which is not available for simple users
			//echo elgg_view_page($title_text, elgg_view_layout("two_sidebar_gc", $params));
		}
	} else {
		forward();
