...
if (isset($vars['entity'])) {
	...
	elgg_log("BRUNO groups:entity ".var_export($vars['entity'], true),'NOTICE');
	elgg_log("BRUNO groups:access ACCESS ".ACCESS_PUBLIC." ".ACCESS_PRIVATE,'NOTICE');
	elgg_log("BRUNO groups:access $membership $access",'NOTICE');
	...
	elgg_log("BRUNO groups:access after if$membership $access",'NOTICE');
...
if ($group_profile_fields > 0) {
	foreach ($group_profile_fields as $shortname => $valtype) {
		...
		elgg_log("BRUNO groups:fields  ".$shortname." ".$vars['entity']->$shortname,'NOTICE');
		echo '<div><label>';
...
if ($tools) {
	...
	//	if ($group_option->name == 'blog') $value='no';
?>	
<div>
	<label>
