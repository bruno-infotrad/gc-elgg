...
$(document).ready(function() {
        ...
        $("#elgg-input-searchgroup").live('keyup', function(){
                if ($.trim($("#elgg-input-searchgroup").val())) {
                        $('#group-search-submit .button-text').css('background','url('+elgg.get_site_url()+'mod/gc_theme/views/default/images/icon-search-active.png) no-repeat 0 0');
                        $('#group-search-submit').css('pointer-events','all');
                        $('#group-search-submit').removeAttr('disabled','disabled');
                } else {
                        $('#group-search-submit .button-text').css('background','url('+elgg.get_site_url()+'mod/gc_theme/views/default/images/icon-search-inactive.png) no-repeat 0 0');
                        $('#group-search-submit').css('pointer-events','none');
                        $('#group-search-submit').attr('disabled','disabled');
                };
        });
