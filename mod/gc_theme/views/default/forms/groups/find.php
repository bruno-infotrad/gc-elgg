...
$params = array(
	...
	'value' => elgg_echo('gc_theme:search:groups'),
);
...
$(document).ready(function() {
        ...
        $("#elgg-input-searchgroup").live('keyup', function(){
                if ($.trim($("#elgg-input-searchgroup").val())) {
                        $('#group-find-submit .button-text').css('background','url('+elgg.get_site_url()+'mod/gc_theme/views/default/images/icon-search-active.png) no-repeat 0 0');
                        $('#group-find-submit').css('pointer-events','all');
                        $('#group-find-submit').removeAttr('disabled','disabled');
                } else {
                        $('#group-find-submit .button-text').css('background','url('+elgg.get_site_url()+'mod/gc_theme/views/default/images/icon-search-inactive.png) no-repeat 0 0');
                        $('#group-find-submit').css('pointer-events','none');
                        $('#group-find-submit').attr('disabled','disabled');
                };
        });
