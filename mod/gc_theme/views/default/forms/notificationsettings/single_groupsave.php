...
if ($group) {
	...
	foreach($NOTIFICATION_HANDLERS as $method => $foo) {
		...
		$fields = '<h4>'.elgg_echo('gc_theme:single_group_notification').'</h4>';
		//$fields = '';
		$i = 0;
		...
		foreach($NOTIFICATION_HANDLERS as $method => $foo) {
			...
			$notification_label =  elgg_echo('gc_theme:notification:method:'.$method);
			$notification_helper =  "<div class='togglefieldhelper'>".elgg_echo('gc_theme:notification:methodhelper:'.$method).'</div>';
			$fields .= <<< END
...
var preference_saved = "<?php echo elgg_echo('gc_theme:usersettings:save:ok'); ?>";
