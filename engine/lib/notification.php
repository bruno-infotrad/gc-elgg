...
function object_notifications($event, $object_type, $object) {
	...
	if ($object instanceof ElggEntity) {
		...
		//Special case: if group, batch process in gc_theme
		if (elgg_instanceof(get_entity($object->container_guid),'group')) {
			$session_guid=elgg_get_logged_in_user_guid();
			$gc_notification = new ElggObject();
			$gc_notification->subtype = 'gc_notification';
			$gc_notification->save();
			$gc_notification->objet = $object->getGUID();
			$gc_notification->type_objet = $object_type;
			$gc_notification->event = $event;
			$gc_notification->submitter_guid = $session_guid;
			$gc_notification->processed = false;
			return true;
		}
		// Get config data
